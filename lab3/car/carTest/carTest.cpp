...
BOOST_FIXTURE_TEST_SUITE(Car, CarSetFixture)
	...
	BOOST_FIXTURE_TEST_SUITE(when_turned_on, when_turned_on_)
		...
		BOOST_AUTO_TEST_SUITE(set_gear_test)
			...
			BOOST_AUTO_TEST_CASE(can_turn_neutral_gear_after_any_gear)
			{
				car.SetGear(-1);
				BOOST_CHECK(car.GetGear() == -1);
				car.SetSpeed(13);
